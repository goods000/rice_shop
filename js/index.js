...
function sum(){
	if ($("ul input[checked='checked']").length==$("li").length) {
		...
		$(".bottom input[type=checkbox]").next("img").attr("src","img/choice.png");
	}else{
		$(".bottom input[type=checkbox]").removeAttr("checked");
		$(".bottom input[type=checkbox]").next("img").attr("src","img/noChoice.png");
	}
}
/*判断是否全选*/
/*给单选框或复选框添加样式*/
	...
	if($this.attr('type')=="checkbox"){
		   if ($this.attr('checked')=="checked") {
		   	$this.removeAttr("checked");
		   	 $this.next('img').attr("src","img/noChoice.png");
		   }else{
		   	 $this.attr("checked","checked");
		   	$this.next('img').attr("src","img/choice.png");
		   }
		}
		/*计算总钱数*/
		total();
...
$('.bottom-label input').change(function(){
		if ($(this).attr("checked")=="checked") {
			...
			$(".con input[type='checkbox']").next('img').attr("src","img/noChoice.png");
		}else{
			$(".con input[type='checkbox']").attr("checked","checked");
			$(".con input[type='checkbox']").next('img').attr("src","img/choice.png");
		}
		checkbox($(this));
})
/*底部全选*/
...
$('.list input').change(function(){
	var $list_input=$(this).parents('.list').next('ul').find('input[type=checkbox]');
		if ($(this).attr("checked")==undefined) {
			...
			$list_input.next('img').attr("src","img/choice.png");
		}else{
			$list_input.removeAttr("checked");
			$list_input.next('img').attr("src","img/noChoice.png");
		} 
		checkbox($(this));
		sum();
})
...
$("ul input[type='checkbox']").change(function(){
	...
	if($(this).parents('ul').find("input[checked='checked']").length==$(this).parents("ul").children('li').length){	
		...
		$ul_input.next('img').attr("src","img/choice.png");
	}else{
		$ul_input.removeAttr("checked");
		$ul_input.next('img').attr("src","img/noChoice.png");
	} 
	sum();
})
/*点击加一*/
...
$('.delete').click(function(){
	...
	$('input[type=checkbox]').next("img").attr("src","img/choice.png");
